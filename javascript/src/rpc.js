...
export class RPC extends MessageEmitter {
  ...
  async _send_chunks_http(data, target_id, session_id = null) {
    ...
    this._emit("http_transmission_stats", transmission_stats);
    ...
    try {
      ...
      const download_and_process = async () => {
        try {
          ...
          // Add trusted context
          main = this._add_context_to_message(main);
  ...
  _add_context_to_message(main) {
    if (!main.ctx) {
      ...
      main.ctx = { ...main, ...this.default_context };
    }
    return main;
  }
  ...
  async _send_chunks(data, target_id, session_id) {
    ...
    if (this._enable_http_transmission && data.byteLength >= this._http_transmission_threshold) {
      if (!this._http_message_transmission_available) {
        // Attempt to initialize HTTP transmission if not yet attempted
        try {
          await this._initialize_http_message_transmission();
        } catch (e) {
          // Ignore initialization errors, will fall back to message_cache
        }
      }
      ...
      if (this._http_message_transmission_available) {
        try {
          ...
          console.warn(`HTTP message transmission failed, falling back to message_cache: ${e}`);
