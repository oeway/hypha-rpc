...
describe("HTTP Message Transmission", () => {
  ...
  before(async function () {
    ...
    for (let i = 0; i < maxAttempts; i++) {
      try {
        ...
        console.log(
          `⏳ S3 not ready yet (attempt ${i + 1}/${maxAttempts}): ${error.message}`,
        );
    ...
    if (!s3Available) {
      console.warn(
        "⚠️  S3 service is not available - HTTP message transmission tests will be skipped",
      );
    }
  });
  ...
  it("should properly initialize HTTP transmission configuration", async () => {
    ...
    expect(httpClient.rpc._http_message_transmission_available).to.be.a(
      "boolean",
    );

    console.log(
      `✅ HTTP transmission available: ${httpClient.rpc._http_message_transmission_available}`,
    );
    console.log(
      `✅ HTTP transmission threshold: ${httpClient.rpc._http_transmission_threshold / (1024 * 1024)}MB`,
    );
    console.log(
      `✅ Multipart threshold: ${httpClient.rpc._multipart_threshold / (1024 * 1024)}MB`,
    );
    ...
    if (httpClient.rpc._http_message_transmission_available) {
      ...
      console.log(
        "⚠️  HTTP transmission not available (this is normal if S3 is not configured)",
      );
    }
  }).timeout(30000);
  ...
  it("should use regular WebSocket transmission for small messages below threshold", async () => {
    ...
    const onHttpTransmission = (data) => {
      ...
      console.log(
        `🔗 HTTP transmission event: ${(data.content_length / (1024 * 1024)).toFixed(1)}MB`,
      );
    };
    ...
    console.log(
      `📤 Sending ${(smallData.byteLength / (1024 * 1024)).toFixed(1)}MB of data...`,
    );
    ...
    console.log(
      `✅ Data echoed successfully in ${transmissionTime.toFixed(2)}s`,
    );
    expect(result).to.deep.equal(smallData);

    // Verify NO HTTP transmission was used
    expect(httpTransmissionEvents).to.have.length(
      0,
      "HTTP transmission should not be used for small messages",
    );
    console.log(
      "✅ Confirmed: Small message used regular WebSocket transmission",
    );
  }).timeout(30000);
  ...
  it("should use single upload HTTP transmission for medium messages", async function () {
    ...
    if (!httpClient.rpc._http_message_transmission_available) {
      console.warn(
        "⚠️  S3/HTTP message transmission is not available - skipping test",
      );
      this.skip();
      return;
    }
    ...
    const onHttpTransmission = (data) => {
      ...
      console.log(
        `   Method: ${data.transmission_method}, Parts: ${data.part_count}`,
      );
    ...
    console.log(
      `📤 Sending ${(mediumData.byteLength / (1024 * 1024)).toFixed(1)}MB of data...`,
    );
    ...
    console.log(
      `✅ Data processed successfully in ${transmissionTime.toFixed(2)}s`,
    );
    ...
    expect(httpTransmissionEvents).to.have.length(
      1,
      "HTTP transmission should be used for medium messages",
    );
    ...
    console.log(
      "✅ Confirmed: Medium message used single upload HTTP transmission",
    );
  }).timeout(30000);
  ...
  it("should use multipart upload HTTP transmission for large messages", async function () {
    ...
    const onHttpTransmission = (data) => {
      ...
      console.log(`   Used multipart: ${data.used_multipart}`);
      console.log(
        `   Part size: ${(data.part_size / (1024 * 1024)).toFixed(1)}MB`,
      );
    };
    ...
    console.log(
      `📤 Sending ${(largeData.byteLength / (1024 * 1024)).toFixed(1)}MB of data...`,
    );
    ...
    console.log(
      `✅ Data analyzed successfully in ${transmissionTime.toFixed(2)}s`,
    );
    ...
    expect(httpTransmissionEvents).to.have.length(
      1,
      "HTTP transmission should be used for large messages",
    );
    const event = httpTransmissionEvents[0];
    ...
    expect(event.part_count).to.be.at.least(
      3,
      `Expected at least 3 parts for 15MB, got ${event.part_count}`,
    );
    console.log(
      "✅ Confirmed: Large message used multipart upload HTTP transmission",
    );
  }).timeout(60000);
  ...
  it("should handle typed array transmission", async function () {
    ...
    console.log(
      `📤 Sending typed array: ${largeArray.length} elements, ${(largeArray.byteLength / (1024 * 1024)).toFixed(1)}MB`,
    );
    ...
    console.log(
      `✅ Array processed successfully in ${transmissionTime.toFixed(2)}s`,
    );
    ...
    expect(httpTransmissionEvents).to.have.length(
      1,
      "HTTP transmission should be used for large typed arrays",
    );
    const event = httpTransmissionEvents[0];
    expect(event.transmission_method).to.equal("multipart_upload");
    expect(event.used_multipart).to.be.true;
    ...
    expect(event.part_count).to.be.at.least(
      6,
      `Expected at least 6 parts for 32MB, got ${event.part_count}`,
    );
    console.log(
      "✅ Confirmed: Large typed array used multipart upload HTTP transmission",
    );
  }).timeout(60000);
  ...
  it("should handle string data transmission", async function () {
    ...
    console.log(
      `📤 Sending large string: ${(new TextEncoder().encode(largeString).length / (1024 * 1024)).toFixed(1)}MB`,
    );
    ...
    console.log(
      `✅ String processed successfully in ${transmissionTime.toFixed(2)}s`,
    );
    ...
    expect(httpTransmissionEvents).to.have.length(
      1,
      "HTTP transmission should be used for large strings",
    );
    const event = httpTransmissionEvents[0];
    expect(event.transmission_method).to.equal("multipart_upload");
    expect(event.used_multipart).to.be.true;
    ...
    expect(event.part_count).to.be.at.least(
      3,
      `Expected at least 3 parts for 12MB, got ${event.part_count}`,
    );
    console.log(
      "✅ Confirmed: Large string used multipart upload HTTP transmission",
    );
  }).timeout(60000);
  ...
  it("should fall back to WebSocket when HTTP transmission fails", async () => {
    ...
    const originalAvailable =
      httpClient.rpc._http_message_transmission_available;
    ...
    console.log(
      `📤 Sending ${(largeData.byteLength / (1024 * 1024)).toFixed(1)}MB with HTTP disabled...`,
    );
    ...
    console.log(
      `✅ Data transmitted successfully via fallback in ${transmissionTime.toFixed(2)}s`,
    );
    ...
    console.log(
      "✅ Confirmed: System gracefully falls back to WebSocket when HTTP transmission is unavailable",
    );
  }).timeout(60000);
  ...
  it("should handle concurrent large transmissions", async function () {
    ...
    console.log(
      `✅ All ${datasets.length} transmissions completed in ${totalTime.toFixed(2)}s`,
    );
    ...
    console.log(
      `📊 HTTP transmission events: ${httpTransmissionEvents.length}`,
    );
    for (const event of httpTransmissionEvents) {
      console.log(
        `   - ${(event.content_length / (1024 * 1024)).toFixed(1)}MB: ${event.transmission_method} (${event.part_count} parts, multipart: ${event.used_multipart})`,
      );
    }

    // All transmissions above 1MB should have used HTTP
    const expectedHttpTransmissions = datasets.filter(
      (data) => data.byteLength >= 1024 * 1024,
    ).length;
  ...
  it("should respect configurable HTTP transmission parameters", async function () {
    ...
    const onHttpTransmissionStats = (event) => {
      ...
      console.log(
        `   📊 HTTP transmission event: ${(event.content_length / (1024 * 1024)).toFixed(1)}MB`,
      );
      console.log(
        `      Method: ${event.transmission_method}, Parts: ${event.part_count}`,
      );
      console.log(`      Used multipart: ${event.used_multipart}`);
      console.log(
        `      Part size: ${(event.part_size / (1024 * 1024)).toFixed(1)}MB`,
      );
      console.log(
        `      Multipart threshold: ${(event.multipart_threshold / (1024 * 1024)).toFixed(1)}MB`,
      );
    };
    ...
    console.log(
      "📤 Testing small message (should use HTTP due to lowered threshold)...",
    );
    const smallData = generateTestData(0.5); // 500KB
    ...
    expect(httpTransmissionEvents).to.have.length(
      1,
      "Small message should trigger HTTP transmission with lowered threshold",
    );
    ...
    console.log(
      "   ✅ Small message correctly used HTTP single upload (due to lowered threshold)",
    );
    ...
    expect(httpTransmissionEvents).to.have.length(
      2,
      "Medium message should trigger HTTP transmission",
    );
    ...
    console.log(
      "📤 Testing large message (should use HTTP multipart upload)...",
    );
    ...
    expect(httpTransmissionEvents).to.have.length(
      3,
      "Large message should trigger HTTP transmission",
    );
  ...
  it("should track actual multipart usage rather than computing from thresholds", async function () {
    ...
    const onHttpTransmission = (data) => {
      ...
      console.log(
        `   Transmission time: ${data.transmission_time.toFixed(2)}s`,
      );
    };
    ...
    console.log(
      `📤 Sending ${(testData.byteLength / (1024 * 1024)).toFixed(1)}MB of data...`,
    );
    ...
    const expectedPartCount = Math.ceil(
      event.content_length / (6 * 1024 * 1024),
    );
    ...
    console.log(
      `   - Part count: ${event.part_count} (calculated: ${expectedPartCount})`,
    );
    console.log(`   - Part size: ${event.part_size} bytes`);
    console.log(
      `   - Transmission time: ${event.transmission_time.toFixed(2)}s`,
    );
  }).timeout(90000);
  ...
  it("should support configurable multipart size and parallel uploads", async function () {
    ...
    console.log(
      `✅ Multipart size configured: ${httpClient.rpc._multipart_size / (1024 * 1024)}MB`,
    );
    console.log(
      `✅ Max parallel uploads configured: ${httpClient.rpc._max_parallel_uploads}`,
    );
    ...
    const onHttpTransmission = (data) => {
      ...
      console.log(
        `   - Multipart size: ${data.multipart_size / (1024 * 1024)}MB`,
      );
    ...
    console.log(
      `📤 Sending ${(testData.byteLength / (1024 * 1024)).toFixed(1)}MB of test data`,
    );
    ...
    console.log(
      `✅ Data processed successfully in ${(endTime - startTime) / 1000}s`,
    );
    ...
    // Allow for small differences due to msgpack encoding overhead
    expect(
      Math.abs(lastEvent.content_length - testData.byteLength),
    ).to.be.lessThan(
      1000,
    ...
    expect(lastEvent.part_count).to.equal(
      Math.ceil(lastEvent.content_length / (6 * 1024 * 1024)),
    );
    expect(lastEvent.used_multipart).to.be.true;

    console.log(
      `✅ Multipart configuration verified: ${lastEvent.part_count} parts of ${lastEvent.multipart_size / (1024 * 1024)}MB each`,
    );
  }).timeout(60000);
  ...
  it("should demonstrate parallel upload performance benefits", async () => {
    ...
    for (const config of configs) {
      ...
      try {
        ...
        console.log(
          `📤 Sending ${(testData.byteLength / (1024 * 1024)).toFixed(1)}MB with ${config.max_parallel_uploads} parallel uploads`,
        );
        ...
        const lastEvent =
          httpTransmissionEvents[httpTransmissionEvents.length - 1];
        expect(lastEvent.used_multipart).to.be.true;
        ...
        console.log(
          `   - Transmission time: ${lastEvent.transmission_time?.toFixed(2)}s`,
        );
      } finally {
