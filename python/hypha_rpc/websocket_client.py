...
class WebsocketRPCConnection:
    ...
    async def start_large_message_upload_via_websocket(
        self, total_size: int, total_parts: int
    ) -> str:
        ...
        await self._websocket.send(start_message)
    ...
    async def complete_large_message_upload_via_websocket(self, session_id: str) -> str:
        ...
        # Create a future to wait for the response
        future = asyncio.Future()
        self._pending_upload_completions = getattr(
            self, "_pending_upload_completions", {}
        )
        self._pending_upload_completions[session_id] = future

        # Send complete upload message via WebSocket
        complete_message = json.dumps(
            {"type": "complete_large_message_upload", "session_id": session_id}
        )

        await self._websocket.send(complete_message)

        # Wait for response
        return await future

    async def upload_large_message_via_http(
        self, data: bytes, part_size: int = 5 * 1024 * 1024
    ) -> str:
        """Upload large message via HTTP multipart upload."""
        total_size = len(data)
        total_parts = math.ceil(total_size / part_size)

        # 1. Start upload via WebSocket
        upload_session_id = await self.start_large_message_upload_via_websocket(
            total_size, total_parts
        )

        # 2. Upload parts in parallel via HTTP
        base_url = self.connection_info.get(
            "public_base_url"
        ) or self.connection_info.get("local_base_url")
        if not base_url:
        ...
        encrypted_id = await self.complete_large_message_upload_via_websocket(
            upload_session_id
        )
        return encrypted_id

    async def download_large_message_via_http(
        self,
        encrypted_id: str,
        total_size: int = None,
        part_size: int = 5 * 1024 * 1024,
    ) -> bytes:
        """Download large message via HTTP with optional range requests."""
        base_url = self.connection_info.get(
            "public_base_url"
        ) or self.connection_info.get("local_base_url")
        if not base_url:
            ...
            while not self._closed and not self._websocket.state == State.CLOSED:
                ...
                if isinstance(data, str):
                    ...
                    if data.get("type") == "reconnection_token":
                        ...
                        if (
                            hasattr(self, "_pending_upload_starts")
                            and session_id in self._pending_upload_starts
                        ):
                        ...
                        if (
                            hasattr(self, "_pending_upload_completions")
                            and session_id in self._pending_upload_completions
                        ):
