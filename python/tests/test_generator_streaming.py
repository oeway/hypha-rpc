...
async def test_generator_streaming_basic(websocket_server):
    ...
    try:
        ...
        assert async_results == ["async_item_0", "async_item_1", "async_item_2", "async_item_3", "async_item_4"]
...
async def test_large_message_streaming(websocket_server):
    ...
    try:
        ...
        large_data = np.random.random((600, 600)).astype(np.float32)  # ~1.4MB, good for testing chunking
        print(f"Created test array: {large_data.shape}, {large_data.nbytes} bytes (~{large_data.nbytes/1024/1024:.1f}MB)")
        ...
        await client.register_service({
            ...
            "process_large_data": large_data_service
        })
...
async def test_generator_streaming_compatibility(websocket_server):
    ...
    try:
        ...
        try:
            ...
            supports_v4 = hasattr(manager, 'config') and getattr(manager.config, 'api_version', 3) >= 4
...
async def test_mixed_data_types_generator(websocket_server):
    ...
    try:
        ...
        await client.register_service({
            "id": "mixed-service",
            "mixed_generator": mixed_generator
        })
