...
from hypha_rpc.utils.mcp import create_mcp_from_service, create_mcp_from_workspace, serve_mcp
...
@schema_function
def add(a: int = Field(..., description="First integer to add"), 
       b: int = Field(..., description="Second integer to add")) -> int:
    """Adds two integers together."""
    return a + b


@schema_function
def subtract(a: int = Field(..., description="Integer to subtract from"), 
            b: int = Field(..., description="Integer to subtract")) -> int:
    """Subtracts the second integer from the first."""
    return a - b


@schema_function
def multiply(a: int = Field(..., description="First integer to multiply"), 
            b: int = Field(..., description="Second integer to multiply")) -> int:
    """Multiplies two integers together."""
    return a * b


@schema_function
def divide(a: int = Field(..., description="Numerator"), 
          b: int = Field(..., description="Denominator (cannot be zero)")) -> float:
...
@schema_function
def execute_python(code: str = Field(..., description="The Python code to execute. Use `__result__ = value` to return a value.")) -> str:
...
async def mcp_service_with_all_features(websocket_server):
    ...
    # Connect to the Hypha server
    server = await connect_to_server({"name": "mcp-features-test", "server_url": WS_SERVER_URL})
...
async def test_service_mcp_client(service_mcp):
    ...
    async with Client(service_mcp) as client:
        ...
        if isinstance(tools, list):
            ...
            tool_names = [tool.name if hasattr(tool, 'name') else tool.get("function", {}).get("name") 
                         for tool in tools]
...
async def test_workspace_mcp_calculator_client(workspace_mcp):
    ...
    async with Client(workspace_mcp) as client:
        ...
        subtract_result = await client.call_tool("calculator_subtract", {"a": 10, "b": 4})
...
async def test_workspace_mcp_interpreter_tool_schema(workspace_mcp):
    ...
    async with Client(workspace_mcp) as client:
        ...
        # Get the tools
        tools = await client.list_tools()
        interpreter_tool = [tool for tool in tools if tool.name == "interpreter_execute"][0]
...
async def test_serve_mcp_calculator(hypha_server_with_services):
    ...
    async with Client(mcp_info) as client:
        ...
        # Extract tool names
        if isinstance(tools, list):
            tool_names = [tool.name if hasattr(tool, 'name') else tool.get("function", {}).get("name") for tool in tools]
        elif isinstance(tools, dict):
            if tools and hasattr(next(iter(tools.values())), 'name'):
                tool_names = [tool.name for tool in tools.values()]
            else:
...
async def test_mcp_service_with_all_features(mcp_service_with_all_features):
    ...
    async with Client(mcp) as client:
        ...
        if hasattr(result, 'text'):
            ...
            first_result = result[0]
            result_text = str(first_result.text if hasattr(first_result, 'text') else first_result)
        else:
            result_text = str(result)
        ...
        # FIXME: getting the prompt content is not working
        # assert prompt_content.messages[0].content.text == "This is a test prompt template"
