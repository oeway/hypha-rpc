...
async def test_unlisted_visibility_consistency():
    ...
    try:
        ...
        ws = await connect_to_server(
            {"name": "python-test-client", "server_url": WS_SERVER_URL, "timeout": 10}
        )
        ...
        assert (
            schema["process_message"]["function"]["description"]
            == "Test service with unlisted visibility."
        )
...
def test_schema_structure_matches_openai_format():
    ...
    def check_structure(actual, expected, path=""):
        if isinstance(expected, dict):
            assert isinstance(
                actual, dict
            ), f"Expected dict at {path}, got {type(actual)}"
            for key, expected_type in expected.items():
                assert key in actual, f"Missing key '{key}' at {path}"
                if isinstance(expected_type, type):
                    assert isinstance(
                        actual[key], expected_type
                    ), f"Expected {expected_type} at {path}.{key}, got {type(actual[key])}"
                else:
    ...
    assert (
        properties["tax_rate"]["description"]
